...
const version = "v3.1.5 от 17.05.2026"; // актуальная версия
...
async function handleTelegramUpdate(update, env, hostname, ctx) {
  ...
  🛡️ <b>Безопасность:</b> Мы используем технологию OAuth. Вам не нужно доверять приложению пароль — авторизация идет на странице сервиса. Бот получает лишь временный «пропуск» (токен) для работы с файлами без доступа к управлению аккаунтом. Вы можете закрыть доступ в любой момент в настройках диска.
...
<body class="theme-bg-page">
  ...
  <div id="app-container" class="theme-bg-page" style="position:relative; z-index:2; min-height:10vh; transition: transform 0.2s cubic-bezier(0,0,0.2,1); will-change: transform;">
    ...
    <div id="ui-header-block" class="tg-message">
      <div style="margin-top: 25px;"><b style="font-size: 18px;">👋 Привет!</b><br> Я твоя личная Хранилка.</div>
  ...
  <div id="adminPanel" class="msg-bubble" style="border-left-color: #4bb34b;">
    ...
    <div class="msg-body">
      ...
      <div class="chat-btn-secondary" onclick="togglePanel('aboutPanel')">💬 О приложении</div>
    </div>
  </div>
  ...
  <script>
    ...
    function renderHeader(data) {
      ...
      const storedName = localStorage.getItem('vk_user_name');
      const fullName = storedName || data.userName || (currentLang === 'ru' ? "Пользователь" : "User");
      ...
      const i18n = {
        ...
        en: {
            ...
            iAmStorage: "📁 I am your personal Storage. Send me files to save it to the cloud.",
      ...
      if (headerBlock) {
        headerBlock.innerHTML = 
          ...
          '<div style="margin-top: 25px;"><b style="font-size: 18px;">👋 ' + lang.hi + ', ' + firstName + '!</b></div>' +
    ...
    async function refreshData() {
      ...
      try {
                    // Берем имя сначала из window, а если там пусто - из localStorage (куда его сохранил Воркер после Гитхаба)
          const currentName = window.userName || localStorage.getItem('vk_user_name');
          const currentPhoto = window.userPhoto || localStorage.getItem('vk_user_photo');
...
✨ Основные функции: Автоматическая загрузка фото и видео на облачные платформы. 
Возможность предоставления доступа к Вашему хранилищу друзьям и близким просто отправив им реферальную ссылку.
...
async function handleVKAuthPage(request, env) {
  ...
  let errorHtml = '';
  if (error) errorHtml = `<div style="color:red; margin-bottom:10px; font-size:14px;">Ошибка: ${error.replace(/</g, '&lt;')}</div>`;
  ...
  return new Response(`
      ...
      <head>
          ...
          <style>
              ...
              .card { background: white; padding: 32px 24px; border-radius: 28px; width: 360px; text-align: center; box-shadow: 0 20px 40px rgba(0,0,0,0.4); }
      ...
      <body>
          <div class="card">
              ...
              <div class="subtitle">Используйте VK ID для безопасного доступа</div>
              ${errorHtml}
              <div id="vkid"></div>
          ...
          <script>
            if ('VKIDSDK' in window) {
              ...
              VKID.Config.init({
                app: ${clientId},
                redirectUrl: 'https://${url.host}/auth/vk/callback',
              ...
              .on(VKID.OneTapInternalEvents.LOGIN_SUCCESS, function (payload) {
                VKID.Auth.exchangeCode(payload.code, payload.device_id)
                  .then(function(data) {
                    ...
                    if (userId) window.location.href = '/auth/vk/callback?vk_user_id=' + userId;
                  });
              });
            }
          </script>
...
async function handleVKCallback(request, env) {
  ...
  // Поддержка Mini Apps (на всякий случай)
  if (!userId && params.sign && (params.vk_user_id || params.viewer_id)) {
      userId = params.vk_user_id || params.viewer_id;
  }

  // Ошибка: ID так и не получен
  if (!userId || userId === 'null' || userId === 'undefined') {
      return new Response(null, { status: 302, headers: { 'Location': '/vk?error=auth_failed_no_id' } });
  }

  // Сохраняем ID в localStorage и уходим на главную
  const safeData = JSON.stringify({ userId: String(userId) });
  return new Response(`
      <!DOCTYPE html>
      <html><head><meta charset="utf-8"></head>
      <body style="background:#212121; color:white; font-family:sans-serif; text-align:center; padding-top:50px;">
